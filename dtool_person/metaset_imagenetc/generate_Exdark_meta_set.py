...
try:
    os.makedirs('./data_inc_person/ExDark/meta')
except:
    print('Alread has this path')

ROOT_DIR = "./person_sample_250_images/ExDark"
IMAGE_DIR = "./person_sample_250_images/ExDark"
TARGET_DIR = "./data_inc_person/ExDark/meta"
num_meta_dataset = 50

with open(ROOT_DIR + "/exdark_annotations_sample_img_250.json",'r') as load_f:
...
for num in trange(num_meta_dataset):
    for img_path in list(car_image_id_dict.keys()):
        ...
        if not os.path.exists(os.path.join(TARGET_DIR, str(num), img_path.split('/')[0])):
            os.makedirs(os.path.join(TARGET_DIR, str(num), img_path.split('/')[0]))
