...
try:
    os.makedirs('./data_inc_person/ECP/meta')
except:
    print('Alread has this path')

ROOT_DIR = "./person_sample_250_images/ECP"
IMAGE_DIR = "./person_sample_250_images/ECP"
TARGET_DIR = "./data_inc_person/ECP/meta"
num_meta_dataset = 50

with open(ROOT_DIR + "/ecp_annotations_sample_img_250.json",'r') as load_f:
...
for num in trange(num_meta_dataset):
    for img_path in list(car_image_id_dict.keys()):
        ...
        if not os.path.exists(os.path.join(TARGET_DIR, str(num), '/'.join(img_path.split('/')[:1]))):
            os.makedirs(os.path.join(TARGET_DIR, str(num), '/'.join(img_path.split('/')[:1])))
