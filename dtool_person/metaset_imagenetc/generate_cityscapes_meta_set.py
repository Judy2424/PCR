...
try:
    os.makedirs('./data_inc_person/cityscapes/meta')
except:
    print('Alread has this path')

ROOT_DIR = "./person_sample_250_images/cityscapes"
IMAGE_DIR = "./person_sample_250_images/cityscapes"
TARGET_DIR = "./data_inc_person/cityscapes/meta"
num_meta_dataset = 50

with open(ROOT_DIR + "/cityscapes_annotations_sample_img_250.json",'r') as load_f:
    load_dict = json.load(load_f)

new_dict = {}
new_dict['images'] = list()
new_dict['annotations'] = list()
new_dict['categories'] = [{'supercategory': 'person', 'id': 1, 'name': 'person'}]
img_list = []
...
for num in trange(num_meta_dataset):
    for img_path in list(car_image_id_dict.keys()):
        ...
        if not os.path.exists(os.path.join(TARGET_DIR, str(num), '/'.join(img_path.split('/')[:3]))):
            os.makedirs(os.path.join(TARGET_DIR, str(num), '/'.join(img_path.split('/')[:3])))

        
        cv2.imwrite(os.path.join(TARGET_DIR, str(num), img_path), corrupted_img)
