...
for i in `seq 0 0`;
  ...
  if [ -f "./res_iou_ac_area_filter/${TAG1}/${i}.json" ]; then
    ...
    python tools/test.py configs/retinanet/retinanet_r50_fpn_1x_coco_car.py ./work_dir_ours/r50_retina_car/epoch_36.pth --eval bbox --cfg-option data.test.img_prefix=./data/kaggle/${DATASET}//udacity_self_driving_car_kaggle/data/export data.test.ann_file=${ANN_DIR}  --tag1 ${TAG1} --tag2 ${i} --dropout_uncertainty ${DROP} --drop_layers ${POS_ARRAY}
  fi
done
